...
def test_beam_preserves_directionally_distinct_fork_hypotheses() -> None:
    points = np.array(
        [
            [1.0, 0.0, 0.0],
            [2.0, 0.6, 0.0],
    ...
    terminal_y = np.asarray([path.points[-1, 1] for path in hypotheses])
    assert np.any(terminal_y > 0.5)
    assert np.any(terminal_y < -0.5)


def test_beam_preserves_moderately_separated_fork_hypotheses() -> None:
    points = np.array(
        [
            [1.0, 0.0, 0.0],
            [2.0, 0.35, 0.0],
    ...
    terminal_y = np.asarray([path.points[-1, 1] for path in hypotheses])
    assert np.any(terminal_y > 0.25)
    assert np.any(terminal_y < -0.25)


def test_radius_continuity_is_a_soft_beam_rank_reward() -> None:
    np.testing.assert_allclose(
        _radius_continuity_scores(np.array([0.02, 0.04, np.nan]), 0.02),
        [1.0, 0.5, 0.5],
    )
    common = dict(
        nodes=[np.zeros(3), np.ones(3)],
        current=np.ones(3),
        direction=np.array([1.0, 0.0, 0.0]),
        covered={1, 2, 3},
        growth_arc=2.0,
        supported_arc=2.0,
        support_sum=20.0,
        turn_squared_sum=400.0,
        cumulative_turn_degrees=20.0,
        cumulative_score=2.0,
        local_radius=0.02,
        radius_observations=4,
        steps=4,
    )
    continuous = _TraceState(radius_similarity_sum=4.0, **common)
    discontinuous = _TraceState(radius_similarity_sum=1.0, **common)

    assert _trace_state_rank(
        continuous,
        step_length=1.0,
