...
def test_deferred_child_refinement_crops_parent_side_prefix() -> None:
    junction = np.array([0.0, 0.0, 0.0])
    original_tip = np.array([0.30, 0.20, -0.10])
    refined = np.array(
        [
            [-0.12, 0.00, 0.02],
            [-0.06, 0.00, 0.01],
    ...
    cropped, removed_arc = _crop_refined_deferred_child(
        refined,
        junction=junction,
        original_tip=original_tip,
    )

    np.testing.assert_allclose(cropped[0], junction)
    np.testing.assert_allclose(cropped[-1], original_tip)
    assert removed_arc > 0.10
    assert np.all(cropped[1:, 0] >= 0.0)
