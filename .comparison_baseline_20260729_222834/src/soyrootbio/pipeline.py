...
def _trace_lateral_orders(
    ...
    for order in range(1, max(1, int(max_root_order)) + 1):
        ...
        deferred_parent_candidates: dict[int, RootPath] = {}
        selected_identity = {id(path) for path in selected}
        for path in selected:
            if path.score_components.get("distal_fork_deferred_child", 0.0) <= 0.0:
                continue
            parent_candidate = next(
                (
                    ...
                    and candidate.points is path.parent_points
                ),
                None,
            )
            if (
                parent_candidate is not None
                and id(parent_candidate) in selected_identity
                and order < max_root_order
            ):
                deferred_parent_candidates[id(path)] = parent_candidate
        selected = [
            path
            for path in selected
            if path.score_components.get("distal_fork_deferred_child", 0.0) <= 0.0
            or id(path) in deferred_parent_candidates
        ]
        selected.sort(
            key=lambda path: (
                path.score_components.get("distal_fork_deferred_child", 0.0) > 0.0,
                -float(path.score),
            )
        )
        ...
        for sequence_index, path in enumerate(selected, start=1):
            deferred = (
                path.score_components.get("distal_fork_deferred_child", 0.0)
                > 0.0
            )
            path.order = order + 1 if deferred else order
            path.root_id = (
                f"order{path.order}_{sequence_base + sequence_index:03d}"
            )
        refined: list[RootPath] = []
        for path in selected:
            parent_candidate = deferred_parent_candidates.get(id(path))
            if parent_candidate is not None:
                path.parent_points = parent_candidate.points
                path.parent_id = parent_candidate.root_id
            else:
                path.parent_points = (
                    path.parent_points
            ...
            for traced in traced_paths:
                deferred_junction = (
                    np.asarray(traced.points[0], dtype=float).copy()
                    if traced.score_components.get(
                ...
                deferred_tip = (
                    np.asarray(traced.points[-1], dtype=float).copy()
                    if deferred_junction is not None
                    else None
                )
                ...
                if np.count_nonzero(local_mask) >= 30:
                    ...
                    if len(centered) >= 2:
                        if (
                            deferred_junction is not None
                            and deferred_tip is not None
                        ):
                            centered, removed_arc = (
                                _crop_refined_deferred_child(
                                    centered,
                                    junction=deferred_junction,
                                    original_tip=deferred_tip,
                                )
                            )
                            traced.score_components[
                                "distal_fork_refinement_prefix_removed"
                            ] = (
                                traced.score_components.get(
                                    "distal_fork_refinement_prefix_removed",
                                    0.0,
                                )
                                + removed_arc
...
def _crop_refined_deferred_child(
    refined_path: np.ndarray,
    *,
    junction: np.ndarray,
    original_tip: np.ndarray,
) -> tuple[np.ndarray, float]:
    """Crop parent-side refinement drift while preserving a known fork.

    A deferred beam hypothesis already carries an explicit parent junction.
    Cross-sectional centerline refinement can nevertheless pull its first
    stations backward along the thicker parent because the covered surface
    support is mixed at the fork.  Orient the refined path toward the original
    child tip, project the known junction onto its segments, discard only the
    prefix before that projection, and restore the exact junction as node zero.
    """

    polyline = np.asarray(refined_path, dtype=float)
    anchor = np.asarray(junction, dtype=float)
    tip = np.asarray(original_tip, dtype=float)
    if (
        polyline.ndim != 2
        ...
        or not np.all(np.isfinite(tip))
    ):
        return polyline.copy(), 0.0

    if float(np.linalg.norm(polyline[0] - tip)) < float(
        np.linalg.norm(polyline[-1] - tip)
    ):
    ...
    if np.any(valid):
        parameters[valid] = np.clip(
            np.sum(
                (anchor - polyline[:-1][valid]) * vectors[valid],
                axis=1,
            )
    ...
    segment = int(
        np.argmin(np.linalg.norm(projections - anchor, axis=1))
    )
    segment_lengths = np.sqrt(np.maximum(squared_lengths, 0.0))
    cumulative = np.concatenate([[0.0], np.cumsum(segment_lengths)])
    removed_arc = float(
        cumulative[segment]
        + parameters[segment] * segment_lengths[segment]
    )
    suffix = np.vstack(
        [
            projections[segment],
    ...
    keep = np.concatenate(
        [
            [True],
            np.linalg.norm(np.diff(suffix, axis=0), axis=1) > 1e-12,
        ]
    )
    suffix = suffix[keep]
    if len(suffix) < 2:
        return polyline.copy(), 0.0
    return suffix, removed_arc
