...
def test_failed_job_writes_utf8_error_log_with_traceback_and_safe_config(tmp_path: Path):
    ...
    def runner(job, control, progress):
        ...
        raise RuntimeError("无法处理样本")

    output_dir = tmp_path / "failed-output"
    input_path = tmp_path / "large_sample.ply"
    ...
    assert snapshot.error == "RuntimeError: 无法处理样本"
    ...
    assert "sample: large_sample.ply" in report
    ...
    assert "exception_message: 无法处理样本" in report
    ...
    assert "RuntimeError: 无法处理样本" in report
    scheduler.shutdown()
