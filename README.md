...
Assignment compares the closest polyline-segment projection of each relevant root, then ranks the closest two **distinct root identities**. With local radius evidence, the comparison uses `abs(segment_distance - interpolated_radius)`; missing radii use zero. Final assignment freezes radius profiles from segmented primary support and unshared traced lateral support before comparing roots. The assignment range and ambiguity margin apply to this surface residual. Segment midpoint KD trees grouped by length provide conservative spatial neighborhoods, followed by exact clamped projections; no fixed nearest-node count can hide a competing root. Empty paths are ignored, singleton and repeated nodes are treated as point segments, and numerical ties within the documented quantization use numeric root identity then segment order.

Competition remains occupied support between tracing orders, participates in parent-owned junction resolution and collar reporting, and protects unresolved points from primary patch cleanup and unrelated child claims. `metadata.json` records the metric, radius profiles, bounds and collar competition counts under `distinct_root_competition`. `root_competition.npz` stores `vertex_indices` and the two distinct numeric `root_labels` for competing vertices; map these through `csv/root_label_map.csv`. Evidence refers to geometry before final support-based centerline fitting, and may include vertices subsequently resolved to their parent.
...
After full-resolution assignment, a mesh-connectivity cleanup absorbs small, discrete order-1, uncertain, or unassigned islands that are enclosed by primary-labelled surface and lie within the measured primary-root envelope. It preserves unresolved distinct-root competition and every order-1 root's main surface component, and never reclaims points excluded above the selected base. The applied policy and absorbed patch/vertex counts are recorded under `point_assignment.primary_surface_patch_cleanup` in `metadata.json`.

Primary–o1 ownership competition follows that cleanup, after sampled primary masks have been restored. Around each primary–o1 emergence it measures local parent and child radii against centerline segments, then trims short primary-labelled protrusions only when exposed child geometry and a bounded mesh-geodesic connection to existing child surfaces support the transfer. Radial position and the local child tangent distinguish the exposed body from an internal connector; the insertion coordinate need not coincide with surface onset. Nearly tied sibling claims retain their current ownership. Excluded collar-side points cannot seed or receive a claim, and uncertain excluded points remain unassigned. Region bounds, radius rules, connectivity mode and per-child transfer counts are recorded under `primary_o1_ownership` in `metadata.json`.
