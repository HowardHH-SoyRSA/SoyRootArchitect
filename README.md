...
- removes a lateral whose origin lies above the primary-root top, together with its dependent subtree;
- recomputes orders recursively; and
- assigns stable IDs such as `root-o1-001`.

The invariant is `primary = order 0`; a child of the primary is order 1; every other child is `parent order + 1`. For every lateral order, the stored insertion point must be at or below one immutable primary-top reference along the configured up direction (opposite gravity). That reference is frozen from the selected preprocessing collar/top—whether supplied by manual guidance, endpoint input, or automatic endpoint selection—before segmentation or centreline refitting. Tracing rejects an ineligible parent-centreline insertion before candidate growth and rechecks the fitted origin before that root can seed the next order; `metadata.json` records both counts and the reference under `lateral_origin_constraint`. Topology repair, hierarchy corrections, final attachment, and final validation reuse the same point. Automatic repair removes a violating root and its descendants instead of inventing another attachment. Editor operations reject the same violation atomically. Final fitting reports its potentially shorter observed top separately as `fitted_primary_top_point_normalized`; it never silently moves the immutable biological boundary. The final graph must be connected to the primary, acyclic, and free of missing parents.
...
An **unassigned** point is either deliberately above the selected base or was not claimed by the segmented primary or by the support radius of any selected lateral. This commonly includes shoot/stem remnants, soil or pot fragments, disconnected noise, distant surface regions, or real roots that tracing did not retain. An **uncertain** point is different: it lies within assignment range but is nearly equally close to competing selected roots, so ownership is intentionally withheld. Initial assignment compares exact distances to centerline segments of distinct roots, including sparse segment interiors, and excludes any explicitly marked internal lateral connector from an exposed-surface claim. The later joint-collar pass also applies geometric exposure checks where a connector has not yet been marked. Counts for both states and the two unassigned categories are written to `metadata.json`; their fractions are included in the system summary.

After full-resolution assignment and sampled-mask restoration, **joint root-collar analysis** evaluates the primary and all laterals whose emergence tubes intersect an adaptive crown neighborhood, including higher-order and unsupported roots. Local surface-neighbor spacing and median segment-radius profiles define the collar: its tipward extent integrates `ds/max(local radius, 2*local spacing)` up to eight, and its radial envelope is `3*local radius + 4*local spacing`. These are dimensionless sampling/geometry rules, not a global physical distance. Emergence tubes are sampled along segments, so participation does not depend on skeleton node placement.

All collar ownership costs use the same frozen input labels, with segment distance, radius residual, emergence direction, mesh-neighbor continuity, hierarchy and bounded geodesic support. Canonical root IDs define candidate columns; ties within `0.15` retain the input owner and are reported as unresolved. Exposed lateral support can reclaim primary labels or another lateral's mislabeled surface, while a trunk/exposure gate prevents broad lateral spread. Explicit `body_start_index` connectors and geometrically internal child axes cannot claim exposed surfaces. Arbitration is followed by a simultaneous connectivity check. Above-base points are unassigned barriers; absent a supplied base mask, the first primary cross-section supplies a local boundary. The stage preserves parent relationships and reports unsupported emergence instead of inferring biological parents from mere surface contact.

The complete decision is saved in `metadata.json` under `joint_root_collar` and separately in `collar_qc.json`: collar bounds, participating roots/orders/parents, local radius and spacing profiles, changed vertex indices and owners, topology assessments, and unresolved region indices/bounds. Joint removal checks retain vertices if removing them would cut a donor surface connection or lose the nearest support of an exposed centerline section by more than half a local spacing. These conflicts are reported for review. Unresolved vertices can retain existing assigned labels; the unresolved QC count is distinct from the `-2` uncertain-label count. A mesh is required for ownership propagation. Reproduce the corrected W51683 comparison with `scripts/validate_joint_collar.py --source outputs/w51683_m4-2_20260415_corrected/corrected_bundle --output outputs/joint_collar_new`. To replay the original edited state directly, supply the original automatic bundle and `--replay-editor-log`; the audit copies the log into the output workspace and verifies it against the materialised edited mesh and hierarchy before applying the collar stage. The audit hashes the immutable source, checks root-order permutation replay, and compares frozen geometry plus controlled downstream fitting, traits and exports across all roots. An exclusion-only control separates above-collar removal from ownership changes.

**Local attachment-footprint QC** runs on full-resolution triangles after each traced order, after topology repair, and after final ownership cleanup. Each pass freezes labels before assessing all current roots. The child's radius comes from angularly observed distal exposed-body sections beyond the flare; the junction needs its own observed cross-section and a native mesh route to that body. A candidate parent footprint must be one connected disk with one closed boundary in a manifold neighborhood. Triangle area, parent-axis span, mesh geodesic diameter, and perimeter-based compactness have separate radius-scaled limits. The oblique-emergence factor is capped. Open, nonmanifold, disconnected-body, and poorly observed cases are reported as unresolved without completing missing geometry. Only rejected proximal child interface vertices can become uncertain after final cleanup, and a mesh-connectivity rollback protects the independently supported body; parent surface, other siblings, and existing ambiguous labels are preserved. Before later-order tracing, starts inside a rejected parent footprint are deferred for QC while supported distal branches remain eligible. A rejected footprint also lists nearby root insertions and their simultaneously measured status for parent review. Initial topology repair retains the trace-proposed parent for rejected and neighboring insertions when no independent alternative is established, then records the eventual parent and order; exterior contact alone does not assign a new parent in that constrained case. Results and exact provisional limits are in `metadata.json` under `attachment_constraint`, with final root flags `attachment_footprint_rejected` or `attachment_unresolved`. The `PipelineConfig` fields prefixed `attachment_` expose the limits for calibration against corrected junctions; the defaults are conservative working bounds, not universal biological cutoffs.

**Symmetric surface-patch correction** then evaluates mesh-connected components outside the joint collar domain, which is protected from subsequent component relabeling. Primary, order-1 and higher-order labels follow identical rules there; the largest component has no exemption. Each component competes with neighboring labels using exact centerline-segment distance, a local radius profile, directional continuity, connection to observed body support, vertex count and boundary-edge support. Radius estimates exclude owned points that fit a neighboring axis better, so a large mislabeled patch cannot freely inflate its owner's envelope. Internal connectors identified by `body_start_index` do not supply exposed-body evidence.

Candidate ownership must connect through actual mesh contacts to an axially supported body patch. Every vertex along that route must fit the candidate's radius envelope; excluded, uncertain and unassigned points are barriers. Mesh edges longer than `4*d_bar` are rejected, and point clouds without triangles skip this correction rather than infer links across nearby surfaces. Supported disconnected fragments can remain assigned. The stage changes labels only, without filling gaps or constructing centerlines.

The original components remain available as atomic patches throughout iterative competition, including after neighboring patches acquire the same label. Each accepted move strictly lowers a fixed energy; sweeps continue until no admissible move improves it. Unary evidence is weighted by vertex count, and the total boundary influence on each patch is capped at `0.35` times that count. Tied best claims within `1e-9` per vertex retain the current owner; patch visitation uses minimum vertex index. This provides deterministic convergence to a component-local minimum, not a guarantee of anatomical correctness or a global optimum. A fresh invocation re-estimates radius/body evidence and can make additional changes. Scores, support gates, candidates, moves and convergence are recorded under `point_assignment.surface_patch_correction` in `metadata.json`, replacing `primary_surface_patch_cleanup` and the separate one-way `primary_o1_ownership` pipeline stage. The historical primary–o1 helper remains available for reproducing earlier audits.

After component correction, branch-facing transection trimming can split a primary-labelled protrusion that remains attached to the main primary component. Actual primary–O1 mesh contact determines the facing sector. Tangent-normal sections compare its 80th-percentile radius with sections 2–6 local radii above and below. A directional excess must exceed the radius-, density- and flank-variation thresholds recorded in metadata. Only exterior surface with mesh continuity to the contacted O1 is reassigned; internal insertion geometry, other lateral labels and collar exclusions are preserved. Newly isolated exterior fragments can follow the same child, while cuts that split retained primary support are rejected. Missing two-sided sections, inconsistent flanks, bilateral widening and unreliable primary reference geometry remain explicit review cases. Per-junction sections, decisions and transfers are stored under `branch_facing_transection_trimming` in `metadata.json`. `scripts/trim_junction_bundles.py` applies this pass to existing complete bundles and writes separate corrected exports, replay logs and full validation reports with their existing centerlines held fixed.

Immediately before centerline refitting, **final local surface cleanup** classifies every primary, order-1 and higher-order mesh component with one exposed-body rule: at least 80% of its vertices must fit the local radius envelope, a mesh-connected supported subset must contain at least three vertices, and its projected longitudinal coverage must reach `max(2*d_bar, local radius)`. Supported detached bodies are retained. An unsupported compact lateral island returns to its declared parent only when at least 75% of its boundary contacts that parent, all island vertices fit the parent envelope, and the contact reaches a locally supported parent surface through real mesh edges. A primary island can similarly move to one clear supported neighboring root. Unsupported compact islands without that evidence become unassigned rather than retaining an unsupported root label; they are protected from immediate hole filling in the same cleanup invocation. No largest-component or primary exemption is used.

The cleanup then rebuilds mesh components and evaluates below-collar unassigned holes. Candidate recipients must touch the hole, fit every hole vertex, and provide a path-aligned local surface anchor at that contact. Boundary contact and radius-adjusted surface fit must identify the same winner; total root size is never considered, and local anchor size breaks only an exact evidence tie. Ambiguous holes remain unassigned. Edges longer than `4*d_bar`, uncertain points, above-base vertices and the protected joint-collar domain are barriers. Point clouds without triangles skip the stage. Decisions and component evidence are recorded under `final_surface_cleanup` and `point_assignment.final_surface_cleanup` in `metadata.json`.

**Higher-order primary-contact correction** checks native triangle edges after final surface cleanup and before final attachment assessment. Direct primary contact is permitted only for order-1 laterals. For an order-2 or higher lateral, the contact seam is reassigned to uncertain ownership on the child side when that preserves its owned surface and native contact with its declared parent. If the child-side cut would split or consume the root, all remaining primary-side seam proposals are evaluated together from frozen labels. They are reassigned only if the primary remains connected, every order-1 root retains at least 80% of its primary contact edges and at least one contact edge, and no protected collar vertex moves. This makes the sample's narrow primary-side contacts actionable without arbitrarily assigning tissue to another biological root. A contact that cannot pass either safety gate remains marked `primary_contact_unresolved` for correction review. Every detected case carries `primary_contact_segmentation_error` for provenance; `primary_contact_repaired` identifies those with no remaining contact. Counts, root IDs, reassignment side, and reasons are exported under `higher_order_primary_contact` in `metadata.json`. Triangle-less point clouds have no native contact evidence and are reported as `unresolved_no_mesh`. `scripts/audit_primary_contact_bundle.py <bundle>` simulates this correction read-only for an existing export.

**Discrete parent-contact reconciliation** checks native mesh components again after each final centreline fit. A child-owned patch touching its declared parent is transferred as a whole only if another component independently supports the child's distal exposed body and the frozen final-cleanup report gives the patch one strong, local parent claim. A nearly circumferential patch may join two already observed primary components when both have native mesh edges to the patch; no new edge is created. The fitted attachment assessment independently proposes a mesh-safe release of rejected proximal child seams. Both decisions use the same frozen labels; a competing claim on the same root, or a cut to a proposed recipient parent's seam, withholds the child transfer and reports the conflict. Other disconnected contacts and connected wraps remain assigned with `parent_contact_unresolved` and a specific discrete or wrap flag where applicable. The selected primary top and the original pre-fit paths are reused for each bounded fit pass. Attachment and higher-order primary-contact audits are refreshed after label changes, then traits and exports use the same final labels and fitted paths. Pass decisions, remaining contacts, and the iteration limit appear under `parent_contact_reconciliation` and `attachment_constraint.postfit_reconciliation_passes` in `metadata.json`. `scripts/replay_parent_contact_bundles.py --bundle <bundle> --output <report.json>` audits an existing export without modifying the source bundle.

The pipeline then assesses and refits every centreline from its **final full-resolution owned points**, before traits, figures, CSV, hierarchy JSON and RSML are produced. For the primary root, the earlier guide-informed axis remains a soft position/orientation prior wherever it is inside supported sections; the immutable selected-top reference remains separate from the observed fitted endpoint. Primary stations have a radius-scaled spacing floor. Their transverse observations use triangle-area and angular balancing, reject one-sided walls and mixed flares, and are solved jointly with uncertainty-adaptive curvature regularization. The solution is constrained to observed local section envelopes with supported endpoint constraints, while real prior bends reduce the smoothing penalty. Laterals retain the conservative mesh-geodesic initialization and transverse body fit. Terminal sections use observed support rather than a farthest surface vertex or extrapolated tip.

Disconnected fragments are never joined by invented bridges: the largest connected component defines the body and excluded point counts are reported. Sparse sections use local observed centroids with a QC flag. A gap without usable local support splits the candidate into runs; only the longest supported run is retained. Primary-fit QC records angular coverage, radius-scale turns, backtracking, length inflation, endpoint retreat and displacement from the prior. A primary candidate that fails any plausibility gate is not applied: the prior axis is retained for downstream traits and the report records `fit_qc_passed: false`, `fit_applied: false`, and `traits_geometry_source: retained_prior`. Thus `status: fitted` is never the sole plausibility test. Roots without a measurable body retain one location for identity and correction, while their path-derived traits are unavailable (NaN), not reported as biological zero length. These conservative choices can shorten paths and leave assigned fragments without an exported centreline; inspect `metadata.json` → `final_centerline_fitting` before interpreting those traits.

Each lateral records `body_start_index` and `centerline_assessment` in the hierarchy. Skeleton CSV rows identify `parent_connector`, `preserved_topology`, `retained_prior`, and `exposed_body`; RSML records the body index and separate lengths. Every sampled supported connector section must lie inside its local owned junction support, including parent-owned protrusions. If a final body fit would remove a descendant's legal attachment below the immutable primary top, the fitter retains a locally joining repaired basal prefix or rolls back that parent's fit. A preserved prefix is explicitly flagged as topology evidence, excluded from exposed-surface tests and volume, and reported through `preserved_topology_length` and `topology_path_length`; its total measured `length` is unavailable. A rolled-back path is entirely marked `retained_prior`: its path length remains available for topology, while measured length, exposed-body length, chord, tortuosity, and estimated volume are unavailable. System summaries count unavailable lengths and volumes and mark their numeric sums as incomplete. Other unsupported attachments remain topology references with explicit gap flags and no spatial bridge. Total measured `length` includes accepted connectors; `exposed_body_length` and `parent_connector_length` separate them. Base/tip direction windows and frustum volume use the exposed body, and radius estimates use final full-resolution ownership. Editor reload/export preserves these distinctions and reported gaps. Subsequent point painting marks an assessment stale; geometry edits invalidate it and follow the existing manual-edit workflow.
