...
The pipeline then assesses and refits every centreline from its **final full-resolution owned points**, before traits, figures, CSV, hierarchy JSON and RSML are produced. Mesh-connected support supplies the longitudinal initialization; transverse sections become narrower at bends, with two fitting iterations and one displacement-limited local smoothing pass. Terminal sections use observed support rather than a farthest surface vertex or extrapolated tip. Existing tracing nodes supply orientation and attachment provenance, not the fitted body geometry.

Disconnected fragments are never joined by invented bridges: the largest connected component defines the body and excluded point counts are reported. Sparse sections use local observed centroids with a QC flag. A gap without usable local support splits the candidate into runs; only the longest supported run is retained. Roots without a measurable body retain one location for identity and correction, while their path-derived traits are unavailable (NaN), not reported as biological zero length. These conservative choices can shorten paths and leave assigned fragments without an exported centreline; inspect `metadata.json` → `final_centerline_fitting` before interpreting those traits.

Each lateral records `body_start_index` and `centerline_assessment` in the hierarchy. Skeleton CSV rows identify `parent_connector` and `exposed_body`, and RSML records the body index and separate lengths. Every sampled connector section must lie inside its local owned junction support, including parent-owned protrusions; otherwise the attachment remains a topology reference with an explicit gap flag and no spatial bridge. Total `length` includes accepted connectors; `exposed_body_length` and `parent_connector_length` separate them. Base/tip direction windows and frustum volume use the exposed body, and radius estimates use final full-resolution ownership. Editor reload/export preserves these distinctions and reported gaps. Subsequent point painting marks an assessment stale; geometry edits invalidate it and follow the existing manual-edit workflow.
