...
4. **Prohibit discrete child patches on the parent.** A child root must not have separate, isolated patches of child-owned surface on its parent root. Detect such patches using native mesh connectivity. Correct ownership only where the evidence supports it; otherwise report an unresolved violation rather than inventing a surface bridge, sacrificing the connected exposed child body, or calling the result compliant.
